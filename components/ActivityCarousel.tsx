...
export default function ActivityCarousel({ whatWeDo, logoUrl }: { whatWeDo: any[], logoUrl: string }) {
    ...
    const transformGoogleDriveUrl = (url: string) => {
        ...
        if (id) {
            const transformed = `https://lh3.googleusercontent.com/u/0/d/${id}`;
            console.log(`[Google Drive Transformer] ${url} -> ${transformed}`);
            return transformed;
        }
        return url;
    };
    ...
    const items = whatWeDo?.length
        ? whatWeDo.map(item => ({ ...item, image_url: transformGoogleDriveUrl(item.image_url) }))
        : [
            { image_url: transformGoogleDriveUrl(logoUrl), activity_name: "Volunteer Camp", description: "Sharing happiness with children in remote areas.", year: 2024 },
            { image_url: transformGoogleDriveUrl(logoUrl), activity_name: "Tech Workshop", description: "Learning new technologies together with experts.", year: 2024 },
            { image_url: transformGoogleDriveUrl(logoUrl), activity_name: "Charity Run", description: "Running for a cause to help local communities.", year: 2024 },
        ];
                            ...
                            <div className="bg-white rounded-[28px] p-2 border border-slate-50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all duration-500 group-hover:shadow-[0_15px_45px_rgb(0,0,0,0.08)] group-hover:-translate-y-2">
                                ...
                                <div className="relative w-full h-[190px] rounded-[22px] overflow-hidden bg-slate-100">
                                    <Image
                                        ...
                                        unoptimized={img.image_url?.includes('googleusercontent.com') || img.image_url?.includes('drive.google.com')}
                                    />
